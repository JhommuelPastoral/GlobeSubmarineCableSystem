...
app.delete("/delete-single-cable-cuts/:cutId", async (req, res) => {
  try {
    ...
    const result = await db.query("DELETE FROM cable_cuts WHERE cut_id = ?", [
      cutId,
    ]);
...
app.put("/cable-cuts/:cutId", async (req, res) => {
  ...
  const query = `UPDATE cable_cuts SET ${updates.join(", ")} WHERE cut_id = ?`;
...
// API: Get a single cable cut by id
app.get("/cable-cuts/:cutId", (req, res) => {
  const { cutId } = req.params;
  if (!cutId) {
    return res.status(400).json({ error: "cutId is required" });
  }
  const trimmedId = cutId.trim();
  const query = "SELECT * FROM cable_cuts WHERE TRIM(cut_id) = ? LIMIT 1";

  db.query(query, [trimmedId], (err, results) => {
    if (err) {
      console.error("Error fetching cable cut:", err);
      return res.status(500).json({ error: "Failed to fetch cable cut" });
    }
    if (results && results.length) {
      return res.json(results[0]);
    }

    // Fallback: try a LIKE match to tolerate minor differences (e.g., case or stray spaces)
    db.query(
      "SELECT * FROM cable_cuts WHERE cut_id LIKE ? LIMIT 1",
      [trimmedId],
      (fallbackErr, fallbackResults) => {
        if (fallbackErr) {
          console.error("Error in fallback cable cut lookup:", fallbackErr);
          return res
            .status(500)
            .json({ error: "Failed to fetch cable cut (fallback)" });
        }
        if (fallbackResults && fallbackResults.length) {
          return res.json(fallbackResults[0]);
        }
        return res.status(404).json({ error: "Cable cut not found" });
      }
    );
  });
});
...
app.post("/cable-cuts", async (req, res) => {
  ...
  try {
    ...
    if (source_table || (cable && segment)) {
      const tableName =
        source_table || `${cable.replace("-", "_")}_rpl_${segment}`;
      ...
      try {
        ...
        if (isTgnTable) {
          ...
          const afterCut =
            afterResult && afterResult.length > 0 ? afterResult[0] : null;
          const beforeCut =
            beforeResult && beforeResult.length > 0 ? beforeResult[0] : null;
        ...
        console.log(
          `Could not lookup cable_type from ${tableName}:`,
          err.message
        );
      }
    }
    ...
    const query = `INSERT INTO cable_cuts (${fields.join(
      ", "
    )}) VALUES (${placeholders})`;
  ...
  fs.createReadStream(filePath)
    ...
    .on("end", async () => {
      try {
        ...
        await new Promise((resolve, reject) => {
          db.query(
            "INSERT INTO previous_utilization SELECT * FROM utilization",
            (err) => {
              if (err) reject(err);
              else resolve();
            }
        ...
        const values = results.map((row) => [
          ...
          row.port_b_side,
          row.bearer_id,
          row.globe_circuit_id,
          row.handover_document,
          row.trunk_type,
          row.link,
          row.mbps_capacity,
          row.gbps_capacity,
          row.percent_utilization,
          row.remarks,
        ]);
        ...
        await new Promise((resolve, reject) => {
          db.query(
            ...
            [
              "Cleared previous data and uploaded new utilization data from CSV",
              fileName,
            ],
            (logErr) => {
        ...
        res.json({
          message:
            "Previous data cleared and new CSV data inserted successfully",
          fileName: fileName,
...
app.post("/upload-rpl/:cable/:segment", upload.single("file"), (req, res) => {
  ...
  console.log(
    `Upload request received for cable: ${cable}, segment: ${segment}`
  );
  ...
  db.query(`SHOW TABLES LIKE '${tableName}'`, (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Database error checking table existence" });
    }
    ...
    fs.createReadStream(filePath)
      ...
      .on("end", () => {
        ...
        if (results.length === 0) {
          ...
          return res
            .status(400)
            .json({ message: "No valid data found in CSV file" });
        }
        ...
        db.query(`DELETE FROM ${tableName}`, (deleteErr, deleteResult) => {
          ...
          console.log(
            `Cleared existing data from ${tableName}. Deleted ${deleteResult.affectedRows} rows.`
          );
          ...
          if (cable === "sea-us" && segment === "s2") {
            ...
            values = results.map((row) => [
              ...
              row.event || null,
              row.repeater || null,
              row.cable_line_no || null,
              row.cable_armour_type || null,
              row.cable_armour_length || null,
              row.section_length || null,
              row.total_length || null,
              row.kp || null,
            ...
            values = results.map((row) => [
              ...
              row.cable_type || null,
              row.cable_id || null,
              row.cable_each_event || null,
              row.cable_each_repeater || null,
              row.cable_cumm || null,
              row.route_each_event || null,
              row.route_distance_cumm || null,
              row.slack || null,
              row.burial_depth || null,
          ...
          db.query(insertQuery, [values], (err, insertResult) => {
            ...
            console.log(
              `Successfully inserted ${values.length} records into ${tableName}`
            );
...
app.get("/data-summary", (request, response) => {
  const sql =
    "SELECT gbps_capacity AS gbps, percent_utilization AS percent FROM utilization;";
...
app.get("/average-util", (request, response) => {
  ...
  const previousQuery =
    "SELECT a_side FROM previous_utilization WHERE site = 'IPOP';";
  ...
  db.query(sql, (error, results) => {
    ...
    db.query(avgSql, (avgError, avgResults) => {
      ...
      db.query(previousAvgSql, (prevAvgError, prevAvgResults) => {
        ...
        const avgUtil =
          parseFloat(avgResults[0].avg_utilization?.toFixed(2)) || 0;
        const prevAvgUtil =
          parseFloat(prevAvgResults[0].prev_avg_utilization?.toFixed(2)) || 0;
...
app.get("/sjc-singapore", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'sjc' AND site = 'singapore'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/sjc-japan", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'sjc' AND site = 'japan'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/sjc-hongkong", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'sjc' AND site = 'hongkong' OR site = 'hong kong'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/c2c-singapore", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'c2c' AND site = 'singapore'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/c2c-japan", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'c2c' AND site = 'japan'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/c2c-hongkong", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'c2c' AND site = 'hongkong' OR site = 'hong kong'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/tgnia-singapore", (request, response) => {
  const sql =
    "SELECT * FROM utilization WHERE (cable = 'tgnia' OR cable = 'tgn-ia') AND site = 'singapore'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/tgnia-japan", (request, response) => {
  const sql =
    "SELECT * FROM utilization WHERE (cable = 'tgnia' OR cable = 'tgn-ia') AND site = 'japan'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/tgnia-hongkong", (request, response) => {
  const sql =
    "SELECT * FROM utilization WHERE (cable = 'tgnia' OR cable = 'tgn-ia') AND (site = 'hongkong' OR site = 'hong kong')";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/tgnia", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'tgnia' OR cable = 'tgn-ia'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/sea-us-seattle", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'sea-us' AND site = 'seattle'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.get("/sea-us-la", (request, response) => {
  const sql =
    "SELECT * FROM utilization where cable = 'sea-us' AND site = 'la'";
  db.query(sql, (error, data) => {
    if (error) return response.json(error);
    return response.json(data);
  });
...
app.post("/register", async (req, res) => {
  ...
  db.query(checkEmailSql, [user_email], async (error, results) => {
    ...
    try {
      ...
      db.query(
        insertSql,
        [user_fname, user_lname, user_email, hashedPassword],
        (err, result) => {
          if (err)
            return res
              .status(500)
              .json({ status: 0, message: "Registration Failed" });
...
app.post("/login", async (req, res) => {
  ...
  const sql =
    "SELECT user_id, user_fname, user_lname, user_role, user_password FROM users WHERE user_email = ?";
