...
const SEGMENTS: SegmentData[] = [
  ...
  {
    id: 'S3',
    label: 'S3 - Lay Interface - Songkhla BU (BU2)',
    endpoint: '/sjc-rpl-s3'
  },
  {
    id: 'S4',
    label: 'S4 - Stubbed Cable End - Songkhla BU (BU2)',
    endpoint: '/sjc-rpl-s4'
  },
  {
    id: 'S5',
    label: 'S5 - Songkhla BU (BU2) - Telisai BU (BU3)',
    endpoint: '/sjc-rpl-s5'
  },
  {
    id: 'S6',
    label: 'S6 - Telisai - Telisai BU (BU3)',
    endpoint: '/sjc-rpl-s6'
  },
  {
    id: 'S7',
    label: 'S7 - Telisai BU (BU3) - Nasugbu BU (BU4)',
    endpoint: '/sjc-rpl-s7'
  },
  {
    id: 'S8',
    label: 'S8 - Nasugbu - Nasugbu BU (BU4)',
    endpoint: '/sjc-rpl-s8'
  },
  {
    id: 'S9',
    label: 'S9 - Nasugbu BU (BU4) - Chung Hom Kok BU (BU5)',
    endpoint: '/sjc-rpl-s9'
  },
  {
    id: 'S10',
    label: 'S10 - Chung Hom Kok - Chung Hom Kok (BU5)',
    endpoint: '/sjc-rpl-s10'
  },
  {
    id: 'S11',
    label: 'S11 - Shantou BU (BU6) - Chung Hom Kok BU (BU5)',
    endpoint: '/sjc-rpl-s11'
  },
  {
    id: 'S12',
    label: 'S12 - Shantou - Shantou BU (BU6)',
    endpoint: '/sjc-rpl-s12'
  },
  {
    id: 'S13',
    label: 'S13 - Chikura - Shantou BU (BU6)',
    endpoint: '/sjc-rpl-s13'
  }
];
  ...
  useEffect(() => {
    ...
    const fetchRoutes = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setLoadError(
            err?.message || 'Unable to load SJC routes for simulation.'
          );
        }
      } finally {
...
const getSegmentLength = (segmentId: string) => {
  ...
  const handleSubmit = async () => {
    ...
    const cutId = `sjc${segNum}-${Date.now()}`;
  ...
  const loadingContent = (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
      ...
      <Typography variant="body2">Loading SJC route data…</Typography>
    </Box>
  );
