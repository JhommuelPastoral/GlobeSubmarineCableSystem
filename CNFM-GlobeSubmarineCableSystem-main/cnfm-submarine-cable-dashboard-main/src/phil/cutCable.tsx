...
import { useDeleteCablePhil } from 'src/hooks/useApi';
import {
  Dialog,
  DialogTitle,
  DialogContent,
...
function CutCable({ cableSegment }: CableCutMarkersProps) {
  const map = useMap();
  ...
  const deleteMutation = useDeleteCablePhil();
  ...
  const fetchCuts = async () => {
    const res = await fetch(`${apiBaseUrl}${port}/cable_cuts_phil`, { method: 'GET' });
    if (!res.ok) {
        ...
        <div style="background-color: white; padding: 10px;">
          <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
            ...
            <tr>
              ...
              <td style="text-align: right; padding-bottom: 5px; color: #666;">${markerData.cable_status === null ? "----" : markerData.cable_status}</td>
            </tr>
          </table>
        </div>
        ${
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(
        `${apiBaseUrl}${port}/cable_cuts_phil/${editDialog.cutId}`,
        {
          method: 'PUT',
