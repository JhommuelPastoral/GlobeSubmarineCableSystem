...
import { useDeletedCablesPhil, useDeleteCablePhil } from 'src/hooks/useApi';
...
const DeletedCablesSidebarPhil: React.FC<DeletedCablesSidebarProps> = ({
    onSelectCable,
    lastUpdate,
    setLastUpdate,
    isAdmin = true,
    ...
    const {
        ...
        isRefetching
    } = useDeletedCablesPhil(lastUpdate);

    // Mutation for deleting cables
    const deleteCableMutation = useDeleteCablePhil();
                                        ...
                                        >
                                            {cable.distance ?? 'N/A'} km — {
                                                (() => {
                                                    ...
                                                    const match = cable.cut_id.match(/^([^-]+)-([^-]+)-(\d+)$/);
                                                    let cableName = 'Unknown';
                                                    let cableNumber = '';
                                                    if (match) {
                                                        const prefix = match[1].toLowerCase();
                                                        cableNumber = match[2];
                                                        const fobn1:string[] = ['nasugbo', 'san jose', 'cadiz', 'san remigio' ,'bacong'];
                                                        const fobn2:string[] = ['legaspi', 'duero', 'potuhan', 'maasin' ,'banate', 'capoocan'];
                                                        const ntdn:string[] = ['boracay', 'taytay', 'bu', 'dalahican', 'mansalay', 'hamtik', 'telicphil', 'bacong'];
                                                        if (fobn1.includes(prefix)) cableName = 'FOBN-1';
                                                        else if (fobn2.includes(prefix)) cableName = 'FOBN-2';
                                                        else if (ntdn.includes(prefix)) cableName = 'NTDN';
                                                    }
...
export default DeletedCablesSidebarPhil;